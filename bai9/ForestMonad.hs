...
move :: (Num a, Eq a, Show a, Ord a) => Move -> State (PlayerStatus a) String
move m = do
    status <- get
    let forest = curForest status
        stamina = curStamina status
    if forest == Exit
        then return $ strCurrentChoice status
        else do
            let forestAfterMove = case m of
                ...
                statusAfterMove = PlayerStatus forestAfterMove staAfterMove
            put statusAfterMove
            return $ strCurrentChoice statusAfterMove
...
resolveForest sta forest mvs = 
    let initPlayerStatus = PlayerStatus forest sta
        (messages, _) = runState (forM mvs move) initPlayerStatus
    in last messages
