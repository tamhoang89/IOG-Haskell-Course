...
type Grape = String
san = "Sangiovese" :: Grape
cab = "Cabernet-sauvignon" :: Grape
mer = "Merlot" :: Grape
gar = "Garnacha" :: Grape
...
larrosaRose = Label {grapes = [gar], region = rioja, kind = Rose 14}
...
castiglioni = Label {grapes = [san], region = tuscany, kind = Red 12.5}
...
lePetitHaitLafitte = Label {grapes = [cab, mer], region = bordeaux, kind = Red 13.5}
-- Question 5
...
containsGrape (l:ls) g = (g `elem` grapes l) || (containsGrape ls g)

containsGrape' :: [Label] -> Grape -> Bool
containsGrape' ls = (`elem` concatMap grapes ls)
