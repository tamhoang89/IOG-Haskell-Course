...
uppercaseChar :: String -> Bool
uppercaseChar word = any (\x -> x `elem` ['A' .. 'Z']) word

uppercaseChar' :: String -> Bool
uppercaseChar' = any (`elem` ['A' .. 'Z'])
...
checkCar b = any (\(brand, quantity) -> brand == b && quantity > 0) cars